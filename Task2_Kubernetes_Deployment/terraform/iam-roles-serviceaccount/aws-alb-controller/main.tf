...
terraform {
  backend "s3" {
    ...
    key            = "eks/irsa/aws-alb-controller/terraform.tfstate"
    region         = "ap-south-1"
...
module "eks_iam_role_alb_controller" {
  source  = "cloudposse/eks-iam-role/aws"
  version = "0.10.3"

  namespace                   = "kube-system"
  environment                 = "prod"
  name                        = "${local.name}-${local.component}-albcontrlr"
  delimiter                   = "-"
  aws_account_number          = data.aws_caller_identity.current.account_id
  eks_cluster_oidc_issuer_url = data.terraform_remote_state.eks.outputs.cluster_oidc_issuer_url
  service_account_name        = "*"
  service_account_namespace   = "kube-system"
  aws_iam_policy_document     = data.aws_iam_policy_document.alb_controller.json
